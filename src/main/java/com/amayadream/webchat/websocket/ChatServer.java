...
public class ChatServer {
    ...
    private static Set<ChatServer> webSocketSet = new HashSet<ChatServer>();
//    private static Set<ChatServer> webSocketSet = new CopyOnWriteArraySet<ChatServer>();  // 线程安全的set
    private Session session;    //与某个客户端的连接会话，需要通过它来给客户端发送数据
    private String userid;      //用户名

    private static Set nameSet = new HashSet<>();   //在线列表,记录用户名称
    ...
    public synchronized void onOpen(Session session, EndpointConfig config){
        ...
        webSocketSet.add(this);     //将当前的chatserver对象加入set中
        ChatServer.onlineCount++; //在线数加1;
        // 获取HttpSession
        HttpSession httpSession = (HttpSession) config.getUserProperties().get(HttpSession.class.getName());
        this.userid = (String) httpSession.getAttribute("userid");    //获取当前用户
        nameSet.add(userid);           //将用户名加入在线列表
        routetab.put(userid, session);   //将用户名和session绑定到路由表
        String message = getMessage("[" + userid + "]加入聊天室,当前在线人数为" + onlineCount + "位", "notice", nameSet);
    ...
    public synchronized void onClose(){
        ...
        ChatServer.onlineCount--;   // 在线数减1
        nameSet.remove(userid);        //从在线列表移除这个用户
        routetab.remove(userid);
        String message = getMessage("[" + userid +"]离开了聊天室,当前在线人数为"+ onlineCount +"位", "notice", nameSet);
        broadcast(message);         //广播
    }
     ...
     * @return
     */
    public String getMessage(String message, String type, Set nameSet){
        JSONObject member = new JSONObject();
        member.put("message", message);
        member.put("type", type);
        member.put("list", nameSet);
        return member.toString();   // {k=v,k1=v1}
    }
